    ...
    if len(hook_mtx[stock_id]) > 0:
        ...
        if order_id == hook_mtx[stock_id][0][0]:
            ...
            if len(trade_lists[target_stk_code - 1]) < target_trade_idx:
                logging.info(
                    f"Order {order_id} of stock {stock_id + 1} waiting for target trade {target_trade_idx} of stock {target_stk_code}")
                print(
                    f"Order {order_id} of stock {stock_id + 1} waiting for target trade {target_trade_idx} of stock {target_stk_code}")
                return True
    return False
...
if __name__ == "__main__":
    ...
    process_put_data_in_queue = multiprocessing.Process(target=put_data_in_queue_squeezed, args=(
