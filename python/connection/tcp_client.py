import asyncio
from asyncio import StreamWriter, StreamReader
from python.utils.wirte_logger import get_logger
from python.connection.pollable_queue import PollableQueue
from queue import Queue
import json
import socket

class ClientTCP:
    def __init__(self, order_queue, response_queue):
        #todo: queue让外面传进来
        self.ip_address = ["127.0.0.1", "127.0.0.1"]
        self.exchange2writer = {}
        self.binding_port = 8888
        "0 is normal, differnet number corresponding different stastus"
        self.ports_status = [0, 0, 0]
        self.log = get_logger(__name__, filename='streaming_client')
        self.count = 0
        self.order_queue = order_queue
        self.response_queue = response_queue

    async def send_message(self, writer: StreamWriter):
        '''send info to server'''
        message = self.response_queue.get()
        while message:
            writer.write((message + '\n').encode())
            await writer.drain()
        writer.close()

    async def listen_for_messages(self, reader: StreamReader):
        while (data := await reader.readline()) != b'':
            await self.response_queue.put(data.decode())
        await self.response_queue.put('Server closed connection.')

    async def client_connection(self, host, port):
        "send order to two exchanger? "
        reader, writer = await asyncio.open_connection(host, port)
        asyncio.create_task(self.listen_for_messages(reader))
        asyncio.create_task(self.send_message(writer))
        return reader, writer

    async def reconnection(self, host, port):
        while True:
            self.log.info('Connecting to server {}:{}'.format(host, port))
            try:
                await self.client_connection(host, port)
            except ConnectionRefusedError:
                self.log.info('Connection to server {}:{} failed!'.format(host, port))
            except asyncio.TimeoutError:
                self.log.error('Connection to server {}:{} timed out!'.format(host, port))
            else:
                self.log.error('Connection to server {}:{} is closed.'.format(host, port))
            await asyncio.sleep(2)

    async def run_main(self):
        #for ip in self.ip_address:
        coros = [self.reconnection(host, self.binding_port) for host in self.ip_address]
        await asyncio.gather(*coros)


if __name__ == '__main__':
    order_queue = PollableQueue()
    response_queue = PollableQueue()
    loop = asyncio.get_event_loop()
    Client_run = ClientTCP(order_queue, response_queue)
    loop.run_until_complete(Client_run.run_main())
    loop.close()
